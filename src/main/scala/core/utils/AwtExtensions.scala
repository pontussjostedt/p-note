...
extension (point2D: Vector2)
    ...
    def noZeros(epsilon: Double = 0.01): Boolean = math.abs(point2D.getX()) > epsilon && math.abs(point2D.getY()) > epsilon
...
extension (affine: AffineTransform)
    ...
    def approximateInvertRect(rect: Rectangle): Rectangle =
        val newAffine = affine.createInverse()
        newAffine.createTransformedShape(rect).getBounds()
...
extension (shape: Shape)
    ...
    def intersects(other: Shape): Boolean =
        ...
        //println(s"selfBounds: $selfBounds")
        selfBounds.contains(otherBounds)  || (selfBounds.intersects(otherBounds) && {
    ...
    def optimizedIntersect(other: Shape, maxArea: Int): Boolean = 
        val selfBounds = shape.getBounds()
        val otherBounds = other.getBounds()
        selfBounds.contains(otherBounds)  || (selfBounds.intersects(otherBounds) && {
            val largestArea = math.max(selfBounds.area, otherBounds.area)
            if largestArea > maxArea then
                val scalingFactor = math.sqrt(maxArea / largestArea)
                val transform = Transform(scale = Vector2(scalingFactor, scalingFactor))
                val newShape = transform.createTransformedShape(shape)
                val newOther = transform.createTransformedShape(other)
                //println(s"The largest area was $largestArea, scaling by $scalingFactor")
                //println(s"newShape: ${newShape.getBounds()}")
                //println(s"newOther: ${newOther.getBounds()}")
                newShape.intersects(newOther)
            else
                val upperPointInImageInner = otherBounds.getUpperLeft() - selfBounds.getUpperLeft()
                val image = BufferedImage(selfBounds.getWidth(), selfBounds.getHeight(), BufferedImage.TYPE_INT_ARGB)
                val g2d = image.createGraphics()
    ...
    def g2dScaledOptimizedIntersect(other: Shape, maxWidth: Double = 200, maxHeight: Double = 200): Boolean =
        val selfBounds = shape.getBounds()
        val otherBounds = other.getBounds()
        selfBounds.contains(otherBounds)  || (selfBounds.intersects(otherBounds) && {
            val largestWidth = math.max(selfBounds.getWidth(), otherBounds.getWidth())
            val largestHeight = math.max(selfBounds.getHeight(), otherBounds.getHeight())
            val scaling = Vector2(maxWidth / largestWidth, maxHeight / largestHeight)
...
extension (rect: Rectangle2D)
    def pad(padding: Double): Rectangle =
        Rectangle(rect.getMinX() + padding, rect.getMinY() + padding, rect.getWidth() - padding, rect.getHeight() - padding)
...
extension (rect: Rectangle)
    def area: Double = rect.getWidth() * rect.getHeight()
